...
\TLV
   ...
   m4_asm(LW, r17, r0, 10000)
   m4_asm(ADDI, r1, r0, 10000000)
   m4_asm(SW,   r0, r1, 10000)
   m4_asm(LB,   r5, r0, 10000)
   m4_asm(LBU,  r6, r0, 10000)
   m4_asm(ADD,  r20, r5, r6)
   m4_asm(SW, r0, r20, 10100)
   m4_asm(LW, r17, r0, 10100)
   ...
   |cpu
      ...
      @5
         $ld_full[31:0] = $dmem_rd_data;
         $ld_byte_off[1:0] = >>2$result[1:0];
         // 선택된 바이트 (8비트)
         $ld_b8[7:0] = $ld_byte_off == 2'b00 ? $ld_full[7:0] :
                       $ld_byte_off == 2'b01 ? $ld_full[15:8] :
                       $ld_byte_off == 2'b10 ? $ld_full[23:16] :
                                               $ld_full[31:24];
         // 선택된 하프워드 (16비트)
         $ld_h16[15:0] = >>2$result[1] ? $ld_full[31:16] : $ld_full[15:0];
         $ld_data[31:0] =
              >>2$is_lb  ? {{24{$ld_b8[7]}},  $ld_b8} :
              >>2$is_lbu ? {24'b0,            $ld_b8} :
              >>2$is_lh  ? {{16{$ld_h16[15]}}, $ld_h16} :
              >>2$is_lhu ? {16'b0,            $ld_h16} :
                           $ld_full;

   *passed = |cpu/xreg[17]>>5$value == 0;
   *failed = 1'b0;
